...
test.describe('startup and retirement contracts',()=>{
  ...
  test('publishes version, manifest, and inert retirement workers @smoke',async({request})=>{const version=await request.get('/version.json');await expect(version).toBeOK();await expect(version).toHaveJSON({appVersion:'v0.5.23',aiInterchangeVersion:4,backupSchemaVersion:7});for(const path of ['/manifest.webmanifest','/sw.js','/service-worker.js','/registerSW.js']){const response=await request.get(path);await expect(response).toBeOK();if(path.endsWith('worker.js')||path.endsWith('sw.js'))expect(await response.text()).not.toContain("addEventListener('fetch'");}});
});
