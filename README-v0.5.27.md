...
Cross-browser stabilization preserves Chromium's html-to-image primary path, prefers semantic Canvas2D for Firefox month columns, and gives WebKit/Chromium bounded html2canvas then Canvas2D long-image fallbacks. Capture-ready markers synchronize the staged publication without retries or sleeps. The floating Add Event action chooses Today only when it is visible, otherwise the rolling window's first date.
