...
import { calculateDeleteImpacts, getIntrinsicBlockedProposalKeys, getPatchReadiness, patchProposalKey } from '../lib/aiPatchApply';
...
export const AIView = () => {
  ...
  const blockedKeys = useMemo(() => preview ? getIntrinsicBlockedProposalKeys(app.activeCalendar.data, preview) : new Set<string>(), [preview,app.activeCalendar.data]);
  ...
  useEffect(() => {
    ...
    const intrinsic = getIntrinsicBlockedProposalKeys(app.activeCalendar.data,preview);
  ...
  const toggleRecord = (key: string) => setSelectedRecords(previous => { const next=new Set(previous); if(next.has(key)) next.delete(key); else if(!blockedKeys.has(key)) next.add(key); return next; });
  const toggleGroup = (records: PreviewRecord[]) => setSelectedRecords(previous => { const next=new Set(previous),allSelected=records.every(record=>next.has(record.key)); records.forEach(record=>{if(allSelected)next.delete(record.key);else if(!blockedKeys.has(record.key))next.add(record.key);}); return next; });
