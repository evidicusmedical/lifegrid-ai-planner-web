...
test.beforeEach(async({page})=>{
 ...
 await page.addInitScript(data=>localStorage.setItem('lifegrid_store_v5',JSON.stringify({activeCalendarId:'v0527',calendars:[{id:'v0527',name:'Convergence Calendar',createdAt:'2026-08-30T12:00:00.000Z',data}]})),seed);
...
test('cross-year occurrences retain the canonical source and correct Day Detail date',async({page})=>{await shiftStart(page,8);await expect(page.getByTestId('cell-2026-12-31').getByText('December event')).toBeVisible();await expect(page.getByTestId('cell-2027-01-01').getByText('January event')).toBeVisible();const occurrence=page.getByTestId('cell-2027-01-01').getByTestId('event-pill-cross');await expect(occurrence).toHaveAttribute('data-occurrence-date','2027-01-01');await occurrence.click();await expect(page.getByTestId('grid-content')).toHaveAttribute('data-detail-date','2027-01-01');expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('lifegrid_store_v5')!).calendars[0].data.events.filter((item:any)=>item.id==='cross').length)).toBe(1);});
