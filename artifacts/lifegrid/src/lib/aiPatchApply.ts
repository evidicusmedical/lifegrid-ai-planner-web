...
export function preflightPatch(current: AppData, patch: any, selected?: Set<string>) {
  ...
  for (const group of ['people','peopleSchedule'] as const) for (const id of patch?.[group]?.delete ?? []) if (!selected || selected.has(key(group,'delete',String(id)))) findings.push({ code:'UNSUPPORTED_DELETE_ENTITY', severity:'blocking', section:group, operation:'delete', recordId:String(id), message:`AI deletion is not supported for ${group === 'people' ? 'People' : 'People Schedule'} records.` });
...
export function calculateDeleteImpacts(current: AppData, patch: any, selected?: Set<string>): DeleteImpact[] { const isDeleted=(group:string,id:string)=>selected?.has(key(group,'delete',id))??false; return groups.flatMap(group => (patch?.[group]?.delete ?? []).map((id:string) => { const record:any=(current as any)[collection[group]]?.find((r:any)=>r.id===id); const affectedEvents=group==='tasks'?current.events.filter(e=>!isDeleted('events',e.id)&&(e.linkedTaskIds??[]).includes(id)).length:group==='categories'?current.events.filter(e=>!isDeleted('events',e.id)&&e.category===id).length:group==='projects'?current.events.filter(e=>!isDeleted('events',e.id)&&e.projectId===id).length:0; const affectedTasks=group==='events'?current.tasks.filter(t=>!isDeleted('tasks',t.id)&&(t.linkedEventIds??[]).includes(id)).length:group==='categories'?current.tasks.filter(t=>!isDeleted('tasks',t.id)&&t.category===id).length:group==='projects'?current.tasks.filter(t=>!isDeleted('tasks',t.id)&&t.projectId===id).length:0; const detachedChildren=group==='tasks'?current.tasks.filter(t=>!isDeleted('tasks',t.id)&&t.parentTaskId===id).length:0; const remainingRecurringSiblings=record?.recurringGroupId?((current as any)[collection[group]]??[]).filter((r:any)=>r.id!==id&&r.recurringGroupId===record.recurringGroupId&&!isDeleted(group,r.id)).length:0; const behavior=group==='categories'?`${affectedEvents} surviving Events → Other · ${affectedTasks} surviving Tasks → Other`:group==='projects'?`${affectedTasks} surviving Tasks detached · ${affectedEvents} surviving Events detached`:group==='tasks'?`${affectedEvents} surviving Event links removed · ${detachedChildren} surviving child Tasks detached`:`${affectedTasks} surviving Task links removed`; return {proposalKey:key(group,'delete',id),affectedEvents,affectedTasks,detachedChildren,remainingRecurringSiblings,behavior}; })); }
