...
test('cross-year occurrences retain the canonical source and correct Day Detail date',async({page})=>{await shiftStart(page,8);await expectMonthKeys(page,['2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03','2027-04','2027-05','2027-06','2027-07','2027-08']);await expect(page.getByTestId('grid-content')).toHaveAttribute('aria-busy','false');await expect(page.getByTestId('cell-2026-12-31').getByTestId('event-pill-dec31')).toBeVisible({timeout:15_000});await expect(page.getByTestId('cell-2027-01-01').getByTestId('event-pill-jan1')).toBeVisible({timeout:15_000});const occurrence=page.getByTestId('cell-2027-01-01').getByTestId('event-pill-cross');await expect(occurrence).toHaveAttribute('data-occurrence-date','2027-01-01');await occurrence.click();await expect(page.getByTestId('grid-content')).toHaveAttribute('data-detail-date','2027-01-01');expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('lifegrid_store_v5')!).calendars[0].data.events.filter((item:any)=>item.id==='cross').length)).toBe(1);});

test('Calendar Year from February uses export month lengths, adaptive wrapping, and a real long PNG',async({page},testInfo)=>{await shiftStart(page,1);await openExport(page,'calendarYear');await expectRange(page,'2026-01-01','2026-12-31');await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expect(page.getByTestId('cell-2026-01-31').getByTestId('event-pill-jan31')).toBeVisible();const title=page.getByTestId('cell-2026-03-31').getByTestId('event-pill-mar31');await expect(title).toHaveAttribute('data-export-title-lines',/^[2-9]/);await expect(title).toHaveAttribute('data-publication-event','true');const style=await title.locator('[data-publication-event-title="true"]').evaluate(node=>{const css=getComputedStyle(node);return{whiteSpace:css.whiteSpace,height:(node.parentElement as HTMLElement).offsetHeight,classes:node.className}});expect(style.whiteSpace).toBe('normal');expect(style.height).toBeGreaterThan(10);expect(style.classes).not.toContain('truncate');const geometry=await page.getByTestId('export-month-publication').evaluate(node=>{const publication=node.getBoundingClientRect();const table=node.querySelector('table')!.getBoundingClientRect();return{publicationWidth:publication.width,tableWidth:table.width,left:table.left-publication.left,right:publication.right-table.right};});expect(Math.abs(geometry.left-24)).toBeLessThanOrEqual(2);expect(Math.abs(geometry.right-24)).toBeLessThanOrEqual(2);expect(geometry.publicationWidth-geometry.tableWidth).toBeLessThan(60);}});await expect(page.getByTestId('event-pill-mar31').locator('[data-publication-event-title="false"]')).toHaveClass(/truncate/);});
