...
const longTitle='Annual readiness planning conference and leadership coordination session';
...
test('exact rolling window and calendar-year navigation use twelve semantic months',async({page})=>{
 const sep=['2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03','2027-04','2027-05','2027-06','2027-07','2027-08'];
 await shiftStart(page,8);await expectMonthKeys(page,sep);await expect(page.locator('[data-month-key="2027-09"]')).toHaveCount(0);
 const y2026=Array.from({length:12},(_,i)=>`2026-${String(i+1).padStart(2,'0')}`),y2027=Array.from({length:12},(_,i)=>`2027-${String(i+1).padStart(2,'0')}`);
 await page.getByTestId('button-year-reset').click();await expectMonthKeys(page,y2026);await page.getByTestId('button-year-next').click();await expectMonthKeys(page,y2027);await page.getByTestId('button-year-prev').click();await expectMonthKeys(page,y2026);
});

test('cross-year occurrences retain the canonical source and correct Day Detail date',async({page})=>{
 await shiftStart(page,8);await expectMonthKeys(page,['2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03','2027-04','2027-05','2027-06','2027-07','2027-08']);
 await expect(page.getByTestId('grid-content')).toHaveAttribute('aria-busy','false');
 expect(await page.evaluate(()=>{const events=JSON.parse(localStorage.getItem('lifegrid_store_v5')!).calendars[0].data.events;return ['dec31','jan1','cross'].every(id=>events.some((item:any)=>item.id===id));})).toBe(true);
 const december=page.getByTestId('cell-2026-12-31');await december.scrollIntoViewIfNeeded();await expect(december.getByTestId('event-pill-dec31')).toBeVisible({timeout:15_000});
 const january=page.getByTestId('cell-2027-01-01');await january.scrollIntoViewIfNeeded();await expect(january.getByTestId('event-pill-jan1')).toBeVisible({timeout:15_000});
 const occurrence=january.getByTestId('event-pill-cross');await expect(occurrence).toBeVisible({timeout:15_000});await expect(occurrence).toHaveAttribute('data-occurrence-date','2027-01-01');await occurrence.click();
 await expect(page.getByTestId('grid-content')).toHaveAttribute('data-detail-date','2027-01-01');expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('lifegrid_store_v5')!).calendars[0].data.events.filter((item:any)=>item.id==='cross').length)).toBe(1);
});

test('Calendar Year from February uses export month lengths, adaptive wrapping, and a real long PNG',async({page},testInfo)=>{await shiftStart(page,1);await openExport(page,'calendarYear');await expectRange(page,'2026-01-01','2026-12-31');await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expect(page.getByTestId('cell-2026-01-31').getByTestId('event-pill-jan31')).toBeVisible();const title=page.getByTestId('cell-2026-03-31').getByTestId('event-pill-mar31');await expect(title).toHaveAttribute('data-export-title-lines',/^[2-9]/);await expect(title).toHaveAttribute('data-publication-event','true');const style=await title.locator('[data-publication-event-title="true"]').evaluate(node=>{const css=getComputedStyle(node);return{whiteSpace:css.whiteSpace,height:(node.parentElement as HTMLElement).offsetHeight,classes:node.className}});expect(style.whiteSpace).toBe('normal');expect(style.height).toBeGreaterThan(10);expect(style.classes).not.toContain('truncate');}});await expect(page.getByTestId('event-pill-mar31').locator('[data-publication-event-title="false"]')).toHaveClass(/truncate/);});

test('Q1 from February retains Jan 31 and Mar 31 in a real month-column PNG',async({page},testInfo)=>{await shiftStart(page,1);await openExport(page,'q1');await expectRange(page,'2026-01-01','2026-03-31');await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expectMonthKeys(page,['2026-01','2026-02','2026-03']);await expect(page.getByTestId('cell-2026-01-31').getByText('January final day')).toBeVisible();await expect(page.getByTestId('cell-2026-03-31').getByText(longTitle)).toBeVisible();}});});

test('Custom validates twelve versus thirteen months and marks partial boundary cells',async({page},testInfo)=>{await openExport(page,'custom');const start=page.getByTestId('input-export-start'),end=page.getByTestId('input-export-end');await start.fill('2026-02-28');await end.fill('2027-01-31');await expect(page.getByTestId('button-export-generate')).toBeEnabled();await start.fill('2026-01-31');await end.fill('2027-01-30');await expect(page.getByText(/no more than 12 calendar months/i)).toBeVisible();await expect(page.getByTestId('button-export-generate')).toBeDisabled();await start.fill('2026-09-20');await end.fill('2027-03-10');await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expectMonthKeys(page,['2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03']);await expect(page.getByTestId('cell-2026-09-19')).toHaveAttribute('data-outside-range','true');await expect(page.getByTestId('cell-2027-03-11')).toHaveAttribute('data-outside-range','true');await expect(page.getByText('Inside partial range')).toBeVisible();await expect(page.getByText('Outside before')).toHaveCount(0);await expect(page.getByText('Outside after')).toHaveCount(0);}});});

test('leap-year full Custom publication includes Feb 29 in a real long PNG',async({page},testInfo)=>{await openExport(page,'custom');await page.getByTestId('input-export-start').fill('2028-01-01');await page.getByTestId('input-export-end').fill('2028-12-31');await expect(page.getByTestId('button-export-generate')).toBeEnabled();await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expect(page.getByTestId('cell-2028-02-29').getByText('Leap-day event')).toBeVisible();}});});

test('Category filter remains authoritative for real month publication',async({page},testInfo)=>{await shiftStart(page,8);await openExport(page,'q4');await page.getByTestId('export-category-work').click();await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expect(page.getByText('Project direct')).toBeVisible();await expect(page.getByText('Personal event')).toHaveCount(0);await expect(page.getByText('Never publish')).toHaveCount(0);}});});

test('Project filter includes direct and Task-derived Events in real month publication',async({page},testInfo)=>{await shiftStart(page,8);await openExport(page,'q4');await page.getByTestId('select-export-project').selectOption('project-a');await generate(page,testInfo,{layout:'month-columns',inspect:async()=>{await expect(page.getByText('Project direct')).toBeVisible();await expect(page.getByText('Project derived')).toBeVisible();await expect(page.getByText('December event')).toHaveCount(0);}});});

test('short range preserves every structural date and generates a real PNG',async({page},testInfo)=>{await openExport(page,'custom');await page.getByTestId('input-export-start').fill('2026-08-29');await page.getByTestId('input-export-end').fill('2026-09-04');for(const date of ['2026-08-29','2026-08-30','2026-08-31','2026-09-01','2026-09-02','2026-09-03','2026-09-04'])await expect(page.getByTestId(`targeted-export-day-${date}`)).toBeAttached();await generate(page,testInfo,{layout:'targeted'});});

test('Add Event defaults to the first displayed date only when Today is outside the rolling window',async({page})=>{await shiftStart(page,8);await expectMonthKeys(page,['2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03','2027-04','2027-05','2027-06','2027-07','2027-08']);await page.getByTestId('button-add-event').click();await expect(page.getByTestId('event-sheet').locator('input[type="date"]').first()).toHaveValue('2026-09-01');await page.keyboard.press('Escape');await page.getByTestId('button-month-prev').click();await expectMonthKeys(page,['2026-08','2026-09','2026-10','2026-11','2026-12','2027-01','2027-02','2027-03','2027-04','2027-05','2027-06','2027-07']);await page.getByTestId('button-add-event').click();await expect(page.getByTestId('event-sheet').locator('input[type="date"]').first()).toHaveValue(TODAY);});

test('export panel remains simplified and current month label is not a dead button',async({page})=>{expect(await page.getByTestId('button-month-current').evaluate(node=>node.tagName)).toBe('SPAN');await page.getByTestId('button-export').click();const panel=page.getByTestId(/panel-export/);for(const label of ['Current Grid','Calendar Year','CY Q1','CY Q2','CY Q3','CY Q4','Next 7','Next 14','Next 30','Custom'])await expect(panel.getByText(label,{exact:true})).toBeVisible();for(const label of ['Visible','Expanded','Fast','Sharp','Compact','Detailed'])await expect(panel.getByText(label,{exact:true})).toHaveCount(0);});
