...
function TimeDataReview() {
  const { events, personEvents, activeCalendar } = useAppData();
  ...
  const visible = findings.filter(issue => (kind === 'all' || issue.recordType === kind) && (code === 'all' || issue.code === code));
  const grouped = visible.reduce<Record<string, typeof visible>>((out, issue) => { (out[issue.code] ??= []).push(issue); return out; }, {});
  const blocking = findings.filter(issue => issue.blocking).length; const advisory = findings.length - blocking;
  const open = (issue: typeof findings[number]) => { const record = issue.recordType === 'event' ? events.find(event => event.id === issue.recordId) : personEvents.find(event => event.id === issue.recordId); if (!record) { setResult('This record is no longer available. Refreshing the review.'); refresh(); return; } setOriginKey(issue.key); setEditing({ issue, record }); };
  return <><Section icon={<CalendarDays size={16}/>} title="Time Data Review" subtitle="Active calendar only. Review does not alter data.">
    <p className="text-xs text-muted-foreground">Time Data Review identifies records whose date, time, timezone, or overnight configuration may be incomplete, ambiguous, or invalid. It never changes your data automatically. Open a record in its normal editor, make a correction, then refresh the review to see which issues remain.</p>
    <div className="flex flex-wrap gap-2 text-xs"><b>{findings.length} total</b><span>{visible.length} visible</span><span className="text-destructive">{blocking} blocking</span><span>{advisory} advisory</span></div>
    <div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground mb-2">Recheck the active calendar after making corrections. This updates the issue list but does not change any records.</p><Button className="min-h-11 w-full sm:w-auto" onClick={() => refresh()} disabled={refreshing} aria-label="Refresh Time Data Review" data-testid="refresh-time-review">{refreshing ? 'Refreshing…' : 'Refresh Review'}</Button>{lastRefreshed && <p className="mt-2 text-[11px] text-muted-foreground">Last refreshed: {lastRefreshed.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</p>}<p className="mt-1 text-xs" aria-live="polite">{result}</p></div>
    ...
    {expanded && <><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><select className="min-w-0 h-11 rounded border bg-background px-2" value={kind} onChange={e => setKind(e.target.value)} data-testid="temporal-review-record-filter"><option value="all">All records</option><option value="event">Events</option><option value="person-schedule">People schedule</option></select><select className="min-w-0 h-11 rounded border bg-background px-2" value={code} onChange={e => setCode(e.target.value)} data-testid="temporal-review-issue-filter"><option value="all">All issue types</option>{[...new Set(findings.map(issue => issue.code))].map(value => <option key={value}>{value}</option>)}</select></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => { setKind('all'); setCode('all'); }}>Clear Filters</Button><Button size="sm" variant="outline" onClick={() => setShowAll(!showAll)}>{showAll ? 'Show first five' : 'Show All'}</Button><Button size="sm" variant="outline" onClick={() => setGroups(Object.fromEntries(Object.keys(grouped).map(key => [key, true])))}>Expand All</Button><Button size="sm" variant="outline" onClick={() => setGroups({})}>Collapse All</Button></div>{!visible.length ? <p className="rounded border p-3 text-xs text-muted-foreground">No time-data issues found.</p> : Object.entries(grouped).map(([group, issues]) => { const openGroup = groups[group] ?? false; const shown = showAll || openGroup ? issues : issues.slice(0, 5); return <div key={group} className="rounded border p-2 space-y-2"><button className="min-h-11 w-full text-left font-semibold text-xs" onClick={() => setGroups({ ...groups, [group]: !openGroup })} aria-expanded={openGroup}>{group} ({issues.length})</button>{shown.map(issue => <div key={issue.key} className="flex min-w-0 flex-wrap items-start justify-between gap-2 rounded bg-muted/30 p-2 text-xs" data-testid={`temporal-review-${issue.key}`}><div className="min-w-0 flex-1"><b className="break-words">{issue.title}</b><p className="break-words">{issue.explanation}</p><span className={issue.blocking ? 'text-destructive' : 'text-muted-foreground'}>{issue.severity}</span></div><Button size="sm" className="min-h-11 shrink-0" onClick={() => open(issue)}>Edit</Button></div>)}</div>; })}</>}
