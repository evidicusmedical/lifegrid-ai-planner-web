...
test('export filenames share local timestamp and calendar sanitizer', () => { const source = read('lib/exportFilenames.ts'); for (const term of ['sanitizeExportName', 'formatExportTimestamp', 'lifegrid_${config.descriptor}_${sanitizeExportName(calendarName)}']) assert.ok(source.includes(term)); });
