...
type Props = { event: Event | null; date: string; category: string; project?: string | null; anchor: DOMRect | null; onClose: () => void };
/** Universal desktop Event preview. Touch activation continues to use DayDetailSheet. */
export const DayTypePreview = ({ event, date, category, project, anchor, onClose }: Props) => {
  const panel = useRef<HTMLDivElement>(null); const exitTimer = useRef<number | undefined>(undefined);
  ...
  return <div ref={panel} id={`day-type-preview-${event.id}`} role="dialog" tabIndex={0} aria-label={`Event preview: ${event.title}`} data-testid="grid-event-preview"
    ...
    onPointerEnter={() => { if (exitTimer.current) window.clearTimeout(exitTimer.current); }} onPointerLeave={() => { exitTimer.current = window.setTimeout(onClose, 180); }}
