...
export const emptyCalendarData = (): AppData => ({ events: [], tasks: [], personEvents: [], people: [], projects: [], milestones: [], categories: [{ ...OTHER_CATEGORY }] });
export const createCalendarSeed = (mode: CalendarSeedMode, source: AppData): AppData => {
  ...
  if (mode === 'copy-structure') return { ...emptyCalendarData(), categories: cloneData(source.categories), people: cloneData(source.people), projects: cloneData(source.projects), milestones: cloneData(source.milestones) };
