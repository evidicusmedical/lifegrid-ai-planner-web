...
test.describe('startup and retirement contracts',()=>{
  ...
  for(const route of routes) test(`loads ${route} @smoke`,async({page})=>{await page.goto(route);await expect(page.getByTestId('app-header')).toContainText('v0.5.29');});
  test('publishes version, manifest, and inert retirement workers @smoke',async({request})=>{const version=await request.get('/version.json');await expect(version).toBeOK();const versionPayload=await version.json();expect(versionPayload).toEqual({appVersion:'v0.5.29',aiInterchangeVersion:5,backupSchemaVersion:7});for(const path of ['/manifest.webmanifest','/sw.js','/service-worker.js','/registerSW.js']){const response=await request.get(path);await expect(response).toBeOK();if(path.endsWith('worker.js')||path.endsWith('sw.js'))expect(await response.text()).not.toContain("addEventListener('fetch'");}});
});
