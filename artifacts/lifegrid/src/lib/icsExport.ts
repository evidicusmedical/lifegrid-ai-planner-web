...
export const buildIcsExport = (app: Pick<AppData, 'categories'>, events: Event[], options: IcsOptions = {}): string => {
 ...
 events.filter(e => e.timeStatus !== 'unknown' || options.includeUnknown).filter(e => e.timeStatus !== 'approximate' || options.includeApproximate).slice().sort((a,b) => a.date.localeCompare(b.date)).forEach(e => {
  ...
  lines.push(`SUMMARY:${escape(e.title)}`); if (e.timeStatus === 'approximate') lines.push('X-LIFEGRID-APPROXIMATE:TRUE'); if (e.timeStatus === 'unknown') lines.push('X-LIFEGRID-TIME-UNKNOWN:TRUE'); const cat = app.categories.find(c => c.id === e.category)?.label ?? e.category; lines.push(`CATEGORIES:${escape(cat)}`); if (e.notes) lines.push(`DESCRIPTION:${escape(e.notes)}`); lines.push('END:VEVENT');
