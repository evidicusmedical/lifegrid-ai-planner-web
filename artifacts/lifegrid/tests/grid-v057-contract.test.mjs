...
test('v0.5.9 retains release compatibility versions', () => {
  assert.match(read('src/lib/version.ts'), /APP_VERSION = 'v0\.5\.9'/);
...
test('Grid transition keeps private development diagnostics and staged shell', () => {
  ...
  assert.match(grid, /gridReady/);
  assert.match(grid, /aria-busy=\{!gridReady\}/);
  assert.match(grid, /lifegrid:grid-dom-start/);
  assert.match(grid, /lifegrid:grid-dom-complete/);
