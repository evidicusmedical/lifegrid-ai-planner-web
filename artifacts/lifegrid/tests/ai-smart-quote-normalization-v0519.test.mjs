...
test('typographic v4 fixture reaches existing parsing and preflight unchanged',()=>{
  ...
  assert.equal(parsed.tasks.add[1].parentTaskId,null); assert.deepEqual(parsed.events.add[0].linkedTaskIds,[]); assert.equal(parsed.events.add[0].recurringGroupId,'home-series');
...
test('realistic asymmetric fixture preserves linked records, dates, warnings, and apostrophes',()=>{
  ...
  assert.equal(parsed.categories.add[1].color,'#2563eb'); assert.equal(parsed.tasks.add[1].parentTaskId,null); assert.equal(parsed.events.add[0].endDate,'2026-08-01'); assert.equal(parsed.events.add[0].recurringGroupId,'relationship-series'); assert.deepEqual(parsed.events.add[0].linkedTaskIds,[]); assert.ok(parsed.warnings.some(w=>w.includes('Jon’s')));
});
