...
export function TemporalFields({ prefix, date, startTime, endTime, endDate, timeStatus, timeZoneMode, timeZone, displayTimeZone, onChange }: {
  ...
  return <div className="rounded-xl border border-border p-3 space-y-3">
    ...
    <p className="text-[11px] text-muted-foreground leading-relaxed" data-testid={test('time-type-explanation')}>{({ 'all-day': 'A confirmed date-based commitment with no clock time. It remains on the same calendar date in every timezone.', timed: 'A confirmed start and end time. Choose a specific timezone for a fixed real-world instant, or Floating when it should remain at the same local clock time wherever you are.', unknown: 'The date is known, but the time is not. It is not treated as a full-day commitment inside LifeGrid.', approximate: 'The time is an estimate rather than a confirmed appointment. LifeGrid marks it as approximate in views and exports.' } as Record<TimeStatus, string>)[timeStatus]}</p>
    {clocked && <>
      ...
      <p className="text-[11px] text-muted-foreground leading-relaxed">{timeZoneMode === 'zoned' ? 'The event represents a fixed instant and may display at a different local time or date when the calendar display timezone changes.' : 'The event stays at the entered clock time and does not move when the calendar display timezone changes.'}</p>{timeZoneMode === 'zoned' && <select value={timeZone} onChange={e => onChange({ timeZone: e.target.value })} className="w-full h-9 rounded border bg-background px-2 text-sm" data-testid={test('timezone')}>{TIME_ZONES.map(z => <option key={z}>{z}</option>)}</select>}
    </>}
