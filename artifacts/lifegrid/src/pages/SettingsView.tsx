...
import { compareReviewFindings, quickTemporalConfirmation, reviewCounts, reviewRefreshMessage, toFlaggedReviewItems } from '../lib/timeReview';
...
function CalendarVersions() {
  const { calendars, activeCalendarId, activeCalendar, switchCalendar, createCalendar, duplicateCalendar, renameCalendar, deleteCalendar, updateCalendarDisplayTimeZone, canResetActiveCalendarToTrulyEmpty, resetActiveCalendarToTrulyEmpty } = useAppData();
...
function TimeDataReview() {
  ...
  const [kind, setKind] = useState('all'); const [code, setCode] = useState('all');
  // Review analysis is intentionally deferred: opening Settings must render its shell
  // without scanning temporal records. Cached findings live for this mounted view.
  const [findings, setFindings] = useState<any[]>([]);
  const [calendarId, setCalendarId] = useState(activeCalendar.id); const [expanded, setExpanded] = useState(false);
  const [reviewLoaded, setReviewLoaded] = useState(false);
  const [showAll, setShowAll] = useState(false); const [groups, setGroups] = useState<Record<string, boolean>>({});
  const [lastRefreshed, setLastRefreshed] = useState<Date | null>(null); const [result, setResult] = useState(''); const [refreshing, setRefreshing] = useState(false);
  const [editing, setEditing] = useState<any>(null); const [originKey, setOriginKey] = useState<string | null>(null);
  const refresh = (postEdit = false) => {
    if (refreshing) return;
    setRefreshing(true);
    try {
      const next = analyzeTemporalReview(events, personEvents); const comparison = compareReviewFindings(findings, next);
      setFindings(next); setReviewLoaded(true); setLastRefreshed(new Date());
      if (postEdit && originKey) setResult(!next.some(issue => issue.key === originKey) ? (comparison.added ? 'Issue resolved, but a new issue was detected.' : 'Issue resolved.') : 'This issue remains.');
      else setResult(reviewRefreshMessage(comparison));
    } catch { setResult('Review could not be refreshed. Previous findings are still shown.'); }
    finally { setRefreshing(false); }
  };
  React.useEffect(() => { if (calendarId !== activeCalendar.id) { setCalendarId(activeCalendar.id); setFindings([]); setReviewLoaded(false); setLastRefreshed(null); setResult('Open or refresh review to analyze this calendar.'); setEditing(null); setOriginKey(null); setExpanded(false); } }, [activeCalendar.id, calendarId]);
  const items = toFlaggedReviewItems(findings);
  const visible = items.filter(issue => (kind === 'all' || issue.recordType === kind) && (code === 'all' || issue.issueCode === code));
  const grouped = visible.reduce<Record<string, typeof visible>>((out, issue) => { (out[issue.issueCode] ??= []).push(issue); return out; }, {});
  const counts = reviewCounts(items);
  const open = (issue: typeof items[number]) => { const record = issue.recordType === 'event' ? events.find(event => event.id === issue.recordId) : personEvents.find(event => event.id === issue.recordId); if (!record) { setResult('This record is no longer available. Refreshing the review.'); refresh(); return; } setOriginKey(issue.issueKey); setEditing({ issue, record }); };
  const confirm = (issue: typeof items[number], choice: 'all-day' | 'unknown') => { const record = issue.recordType === 'event' ? events.find(e => e.id === issue.recordId) : personEvents.find(e => e.id === issue.recordId); if (!record) { refresh(); return; } const update = quickTemporalConfirmation(record, choice); if (issue.recordType === 'event') updateEvent(issue.recordId, update); else updatePersonEvent(issue.recordId, update); setResult(choice === 'all-day' ? 'Confirmed as all day. Rechecking review.' : 'Confirmed as time unknown. Rechecking review.'); window.setTimeout(() => refresh(), 0); };
  const remove = (issue: typeof items[number]) => { if (!window.confirm(`Delete '${issue.title}'? This deletes the ${issue.recordType === 'event' ? 'Event' : 'Person Schedule'} itself, not only this review flag. Recovery requires restoring a backup.`)) return; if (issue.recordType === 'event') deleteEvent(issue.recordId); else deletePersonEvent(issue.recordId); setResult('Record deleted. Rechecking review.'); window.setTimeout(() => refresh(), 0); };
  return <><Section icon={<CalendarDays size={16}/>} title="Flagged Items for Review" subtitle="Active calendar only.">
    <p className="text-xs text-muted-foreground">Review records that contain errors, require clarification, or need confirmation after an import or application update.</p>
    <div className="flex flex-wrap gap-2 text-xs"><b>{reviewLoaded ? `${counts.total} total analyzer findings` : 'Open to analyze current temporal records'}</b>{reviewLoaded && <><span>{visible.length} visible</span><span className="text-destructive">{counts.blocking} blocking</span><span>{counts.advisory} advisory</span><span>0 ignored advisory</span></>}</div>
    <div className="rounded-lg bg-muted/30 p-3"><p className="text-xs text-muted-foreground mb-2">Refresh reruns registered review analyzers and never changes records.</p><Button className="min-h-11 w-full sm:w-auto" onClick={() => refresh()} disabled={refreshing} aria-label="Refresh Flagged Items for Review" data-testid="refresh-flagged-review">{refreshing ? 'Refreshing…' : 'Refresh Review'}</Button>{lastRefreshed && <p className="mt-2 text-[11px] text-muted-foreground">Last refreshed: {lastRefreshed.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</p>}<p className="mt-1 text-xs" aria-live="polite">{result}</p></div>
    <Button size="sm" variant="outline" className="min-h-11" onClick={() => { const next = !expanded; setExpanded(next); if (next && !reviewLoaded) refresh(); }} aria-expanded={expanded} data-testid="time-review-toggle">{expanded ? 'Collapse' : 'Expand'} review</Button>
    {expanded && <><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><select className="min-w-0 h-11 rounded border bg-background px-2" value={kind} onChange={e => setKind(e.target.value)} data-testid="flagged-review-record-filter"><option value="all">All records</option><option value="event">Events</option><option value="person-schedule">People schedule</option></select><select className="min-w-0 h-11 rounded border bg-background px-2" value={code} onChange={e => setCode(e.target.value)}><option value="all">All issue types</option>{[...new Set(items.map(issue => issue.issueCode))].map(value => <option key={value}>{value}</option>)}</select></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => { setKind('all'); setCode('all'); }}>Clear Filters</Button><Button size="sm" variant="outline" onClick={() => setShowAll(!showAll)}>{showAll ? 'Show first five' : 'Show All'}</Button></div>{!visible.length ? <p className="rounded border p-3 text-xs text-muted-foreground">No flagged items.</p> : Object.entries(grouped).map(([group, issues]) => { const openGroup = groups[group] ?? false; const shown = showAll || openGroup ? issues : issues.slice(0, 5); return <div key={group} className="rounded border p-2 space-y-2"><button className="min-h-11 w-full text-left font-semibold text-xs" onClick={() => setGroups({ ...groups, [group]: !openGroup })} aria-expanded={openGroup}>{group} ({issues.length})</button>{shown.map(issue => <div key={issue.issueKey} className="flex min-w-0 flex-wrap items-start justify-between gap-2 rounded bg-muted/30 p-2 text-xs" data-testid={`flagged-review-${issue.issueKey}`}><div className="min-w-0 flex-1"><b className="break-words">{issue.title}</b><p className="break-words">{issue.reason}</p>{issue.question && <p className="break-words font-medium">{issue.question}</p>}<span className="text-destructive">{issue.recordType} · {issue.severity} · {issue.source}</span></div><div className="flex flex-wrap gap-1"><Button size="sm" className="min-h-11" onClick={() => open(issue)}>Edit Record</Button>{issue.allowedActions.includes('confirm-all-day') && <Button size="sm" variant="outline" className="min-h-11" onClick={() => confirm(issue, 'all-day')}>Confirm All Day</Button>}{issue.allowedActions.includes('confirm-time-unknown') && <Button size="sm" variant="outline" className="min-h-11" onClick={() => confirm(issue, 'unknown')}>Confirm Time Unknown</Button>}<Button size="sm" variant="destructive" className="min-h-11" onClick={() => remove(issue)}>Delete Record</Button></div></div>)}</div>; })}</>}
  </Section>{editing?.issue.recordType === 'event' ? <EventSheet isOpen onClose={() => { setEditing(null); setOriginKey(null); }} initialData={editing.record} onSaved={() => { setEditing(null); window.setTimeout(() => refresh(true), 0); }} /> : editing ? <PersonEventSheet isOpen onClose={() => { setEditing(null); setOriginKey(null); }} initialData={editing.record} onSaved={() => { setEditing(null); window.setTimeout(() => refresh(true), 0); }} /> : null}</>;
}
...
function ExportManager() {
  ...
  return <Section icon={<FileText size={16} />} title="Share or publish your schedule" subtitle="These exports are not restorable backups.">
    ...
    <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="secondary" disabled={!!rangeError} onClick={() => { download(buildTextExport(app), exportFilename('text_export', app.activeCalendar.name), 'text/plain'); toast.success('Text export downloaded', { description: 'Human-readable only — not restorable.' }); }}><Download size={14}/> Export readable text</Button><Button size="sm" variant="secondary" disabled={!!rangeError || !events.length} onClick={() => { download(buildIcsExport(app, events, { timeMode }), exportFilename('ics_export', app.activeCalendar.name), 'text/calendar'); toast.success('ICS exported', { description: `${events.length} calendar/grid event${events.length === 1 ? '' : 's'} exported.` }); }}><Download size={14}/> Export ICS calendar</Button></div>
