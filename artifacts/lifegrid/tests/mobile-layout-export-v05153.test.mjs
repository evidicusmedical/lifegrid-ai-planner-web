...
test('release identity and schema/interchange compatibility remain correct', () => { assert.match(read('../src/lib/version.ts'), /APP_VERSION = ["']v0\.5\.21["']/); assert.match(read('../package.json'), /"version": "0\.5\.21"/); assert.match(read('../src/lib/version.ts'), /AI_INTERCHANGE_VERSION = 4/); assert.match(read('../src/lib/backup.ts'), /BACKUP_SCHEMA_VERSION = 7/); });
