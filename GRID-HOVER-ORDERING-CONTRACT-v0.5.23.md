...
The Custom image-export preset is applied through one `exportFilters` state transition, initializes a sensible valid range when empty, exposes `aria-pressed`, and retains values when switching away and back. Its inputs, validation, expanded buckets, targeted weeks, and capture path all consume that same state object; interactions remain contained inside the Export dialog.
