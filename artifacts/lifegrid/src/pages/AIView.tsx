...
export const AIView = () => {
  ...
  const validationBlockedKeys = useMemo(() => { if(!preview) return new Set<string>(); const keys=new Set(recordGroups.flatMap(g=>g.records.map(r=>r.key))); return new Set(getPatchReadiness(app.activeCalendar.data,preview,keys).findings.filter(f=>f.severity==='blocking'&&f.section&&f.section!=='patch'&&f.operation&&f.recordId).map(f=>patchProposalKey(f.section as any,f.operation!,f.recordId!))); },[preview,recordGroups,app.activeCalendar.data]);
  const blockedKeys = new Set([...(dependencyAnalysis ? [...dependencyAnalysis.blocked].filter(([key]) => selectedRecords.has(key)).map(([key]) => key) : []),...validationBlockedKeys]);
  ...
  useEffect(() => {
    ...
    setSelectedRecords(new Set(proposals.filter(proposal => proposal.operation !== 'Delete' && !blockingKeys.has(proposal.key)).map(proposal => proposal.key)));
  ...
  const apply = () => { if (!preview || !readiness?.canApply) { toast.error(readiness?.disabledReason ?? 'Resolve blocking errors before applying.'); return; } const deletes=recordGroups.flatMap(g=>g.records).filter(r=>r.operation==='Delete'&&selectedRecords.has(r.key)); if(deletes.length&&!window.confirm(`Apply destructive AI changes?\n\n${deletes.length} record${deletes.length===1?'':'s'} will be permanently deleted. Related references will be repaired as shown in Preflight.\n\nDelete and Apply ${readiness.selectedCount} Selected Changes`)) return; const selected = filterSelectedUpdate(preview, selectedRecords); try { const warnings = app.applyImportUpdate(selected); toast.success(`Applied ${readiness.selectedCount} selected changes`, { description: warnings.length ? warnings.join(' ') : undefined }); setPreview(null); setResponse(''); } catch (e: any) { toast.error('Transaction failed — no changes were applied.', { description: e.message }); } };
...
type PreviewRecord = { key: string; group: string; operation: 'New' | 'Update' | 'Delete'; label: string; id: string; detail?: string };
const toRecordGroups = (update: ParsedUpdate, current: any): { title: string; records: PreviewRecord[] }[] => {
  ...
  const add = (title: string, entityType: 'categories'|'people'|'projects'|'tasks'|'events'|'peopleSchedule', source: any, detail: (record: any) => string | undefined = () => undefined) => {
    const impacts=new Map(calculateDeleteImpacts(current,{[entityType]:source}).map(i=>[i.proposalKey,i])); const records = ['add', 'update','delete'].flatMap(operation => (source?.[operation] ?? []).map((value: any) => { const id=String(operation==='delete'?value:value.id); const record=operation==='delete' ? current[{categories:'categories',people:'people',projects:'projects',tasks:'tasks',events:'events',peopleSchedule:'personEvents'}[entityType]].find((r:any)=>r.id===id) : value; return { key: patchProposalKey(entityType, operation as any, id), group: title, operation: operation === 'add' ? 'New' as const : operation==='update'?'Update' as const:'Delete' as const, label: operation==='delete' ? (record?.name??record?.title??record?.label??id) : resolveProposalDisplayLabel(entityType, operation as any, record, current), id, detail: operation==='delete'?impacts.get(patchProposalKey(entityType,'delete',id))?.behavior:detail(record) }; }));
