...
function ExportManager() {
  ...
  return <Section icon={<FileText size={16} />} title="Share or publish your schedule" subtitle="These exports are not restorable backups.">
    ...
    <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="secondary" disabled={!!rangeError} onClick={() => { download(buildTextExport(app), exportFilename('text_export', app.activeCalendar.name, new Date(), app.activeCalendar.displayTimeZone), 'text/plain'); toast.success('Text export downloaded', { description: 'Human-readable only — not restorable.' }); }}><Download size={14}/> Export readable text</Button><Button size="sm" variant="secondary" disabled={!!rangeError || !events.length} onClick={() => { download(buildIcsExport(app, events, { timeMode }), buildLifeGridExportFilename({ kind: 'ics_calendar', calendarName: app.activeCalendar.name, generatedAt: new Date(), timeZone: app.activeCalendar.displayTimeZone }), 'text/calendar'); toast.success('ICS exported', { description: `${events.length} calendar/grid event${events.length === 1 ? '' : 's'} exported.` }); }}><Download size={14}/> Export ICS calendar</Button></div>
