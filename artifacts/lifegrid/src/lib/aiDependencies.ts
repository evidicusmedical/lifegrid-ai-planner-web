...
export function analyzeDependencies(update: ParsedUpdate, data: AppData, selected: Set<ProposalKey>): DependencyAnalysis {
  ...
  groups.forEach(group => ['add', 'update'].forEach(operation => ((update[group] as any)?.[operation] ?? []).forEach((r: any, i: number) => {
    ...
    refs(group, r).forEach(([parentGroup, id]) => { const parent = keys.get(`${parentGroup}:${id}`); if (deleting[parentGroup].has(id)) blocked.set(child, `Blocked: ${group.slice(0,-1)} ${r.id} references ${parentGroup.slice(0,-1)} ${id}, but it is selected for deletion`); else if (parent) { const list = dependents.get(parent) ?? []; list.push(child); dependents.set(parent, list); if (!selected.has(parent)) blocked.set(child, `Blocked: ${parentGroup.slice(0, -1)} ${id} is not selected`); } else if (!existing[parentGroup].has(id)) blocked.set(child, `Blocked: ${parentGroup.slice(0, -1)} ${id} does not exist`); });
  })));
