...
test('release version markers remain aligned', () => {
  assert.match(read('src/lib/version.ts'), /APP_VERSION = [\"']v0\.5\.25[\"']/);
  assert.equal(JSON.parse(read('public/version.json')).appVersion, 'v0.5.25');
  assert.match(read('index.html'), /lifegrid-app-version" content="v0\.5\.25"/);
  assert.equal(JSON.parse(read('package.json')).version, '0.5.25');
});
