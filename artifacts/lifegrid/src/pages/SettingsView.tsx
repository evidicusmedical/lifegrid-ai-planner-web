...
function TimeDataReview() {
  ...
  const [findings, setFindings] = useState(() => analyzeTemporalReview(events, personEvents));
  const [calendarId, setCalendarId] = useState(activeCalendar.id); const [expanded, setExpanded] = useState(findings.some(issue => issue.blocking));
  ...
  const refresh = (postEdit = false) => {
    ...
    try {
      ...
      setFindings(next); setLastRefreshed(new Date());
  ...
  React.useEffect(() => { if (calendarId !== activeCalendar.id) { setCalendarId(activeCalendar.id); const next = analyzeTemporalReview(events, personEvents); setFindings(next); setLastRefreshed(null); setResult('Review loaded for the active calendar.'); setEditing(null); setOriginKey(null); setExpanded(next.some(issue => issue.blocking)); } }, [activeCalendar.id, calendarId, events, personEvents]);
  ...
  return <><Section icon={<CalendarDays size={16}/>} title="Flagged Items for Review" subtitle="Active calendar only.">
    ...
    <div className="flex flex-wrap gap-2 text-xs"><b>{counts.total} total analyzer findings</b><span>{visible.length} visible</span><span className="text-destructive">{counts.blocking} blocking</span><span>{counts.advisory} advisory</span><span>0 ignored advisory</span></div>
    ...
    <Button size="sm" variant="outline" className="min-h-11" onClick={() => setExpanded(!expanded)} aria-expanded={expanded} data-testid="time-review-toggle">{expanded ? 'Collapse' : 'Expand'} review</Button>
